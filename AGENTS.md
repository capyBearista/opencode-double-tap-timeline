...
OpenCode is an open-source coding agent that runs in the terminal and helps you work with a codebase through natural-language instructions.

In current AI-engineering usage, a “coding harness” or “agent harness” means the software around the model: prompts, tools, sandboxing, context management, state persistence, verification, and recovery. Put another way, the harness is what turns a raw LLM into something that can reliably carry out multi-step work instead of just answering one prompt.

So the cleanest description is: OpenCode is an open-source coding agent, and in harness terms it is the layer that wraps a model with the workflows, permissions, and execution logic needed to plan changes, make edits, and keep long-running coding tasks on track.
